(ns sapiens.main
  "Kotodama WASM entrypoint for the Sapiens clean-room actor (L4) — Clojure port.
  ...
  facts (`sapiens.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sapiens")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "sapiens_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                   :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "sapiens_loa"
    :fields [:borrowerId :principal :rate :termMonths :status] :required [:principal :rate]
    :coerce {:principal :float :rate :float :termMonths :int} :refs {}}
   {:entity "Policy"      :plural "policies"     :id-prefix "sapiens_pol"
    :fields [:holderId :type :premium :coverage :status] :required [:type :premium]
    :coerce {:premium :float :coverage :float}  :refs {}}
   {:entity "Claim"       :plural "claims"       :id-prefix "sapiens_cla"
    :fields [:policyId :amount :status :filedAt] :required [:amount :status]
    :coerce {:amount :float}                    :refs {:policyId "Policy"}}
   {:entity "Transaction" :plural "transactions" :id-prefix "sapiens_tra"
    :fields [:accountId :amount :type :postedAt] :required [:amount :type]
    :coerce {:amount :float}                    :refs {:accountId "Account"}}
   {:entity "Customer"    :plural "customers"    :id-prefix "sapiens_cus"
    :fields [:name :taxId :kycStatus]          :required [:name :kycStatus]
    :coerce {}                                  :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"sapiens.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sapiens-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sapiens-compat/ready)
